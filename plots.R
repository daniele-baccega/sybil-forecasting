...
#   - N:                      total population
SIRD_check <- function(dir_name, SIRD, infection_rates, rec_rates, fat_rates, vac_rates, immunization_end_rate, N, response){
  ...
  for(t in 1:(n-1)){
    S_local[t+1] <- S_local[t] - infection_rates[t] * response$average_mobility[t] * I_local[t] * S_local[t] / N + R_local[t] * immunization_end_rate - S_local[t] * vac_rates[t] + V_local[t] * immunization_end_rate
    I_local[t+1] <- I_local[t] + infection_rates[t] * response$average_mobility[t] * I_local[t] * S_local[t] / N - I_local[t] * (rec_rates[t] + fat_rates[t])
...
plot_SIRD_evolution <- function(df_local, n, n_ref, dir_name, time_step, ref_data_flag, final_date, infection_rates, SIRD, SIRD_ref, variants){
  if(variants){
    ...
    plot <- ggplot(df_plot) +
      ...
      facet_wrap(~place, scales="free_y") +
      scale_colour_manual(values=c("#F3474D", "#6B95DB", "#000000")) +
      theme(panel.spacing = unit(1, "cm"), legend.key.size = unit(2, 'cm'), axis.text=element_text(size=45), axis.title=element_text(size=50, face="bold"), plot.title = element_text(size=50, face="bold"), legend.title=element_text(size=50, face="bold"), legend.text=element_text(size=48), strip.text.x = element_blank()) +
      labs(title=paste0(time_step, " days"), x="Date", y="Population", color="Variants", linetype="Type") +
      scale_y_continuous(labels = label_scientific())
    save(plot, file = paste0(dir_name, "/RData/SIRD_forecast_", time_step, "_days.RData"))
  }
