...
# Coronasurveys estimates, fixed recovery rates and vaccination
...
download_files_and_load_data <- function(country_long, global_initial_date, global_final_date, variants, variants_to_disregard = list(), variants_aggregated = list(), variants_aggregated_names = list()){
  ...
  if(!dir.exists("aggregatesUMD/"))
    system("git clone git@github.com:daniele-baccega/aggregatesUMD.git")
  ...
  df_disease_ref_init <- df_disease_ref_init %>%
    mutate(confirmed = na.approx(confirmed, na.rm = FALSE), vaccines = na.approx(vaccines, na.rm = FALSE), deaths = na.approx(deaths, na.rm = FALSE))
  
  df_disease_ref_init <- df_disease_ref_init %>%
    filter(!is.na(confirmed), !is.na(deaths), !is.na(vaccines), date <= global_final_date, date >= global_initial_date)
...
compute_data <- function(df_disease_ref, df_variants_ref, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, daily_spline, country){
  ...
  df_disease_ref <- df_disease_ref %>%
    mutate(total_deaths = deaths, total_vaccines = vaccines) %>%
    select(date, total_deaths, total_vaccines, population) %>%
    mutate(new_deaths = diff(c(0, total_deaths)), new_vaccines = diff(c(0, total_vaccines))) %>%
  ...
  df_disease_ref <- rbind(data.frame(date=as.Date(df_disease_ref$date[1])-1, total_deaths=df_disease_ref$total_deaths[1]-df_disease_ref$new_deaths[1], population= df_disease_ref$population[1], new_deaths=0, new_cases=0, new_vaccines=0, total_vaccines=df_disease_ref$total_vaccines[1], total_cases=coronasurveys_data$p_cli[1]), df_disease_ref)
  ...
  SIRDS_initial_marking <- c(unique(df_disease_ref$population) - coronasurveys_data$p_cli[1] - df_disease_ref$total_deaths[1], df_disease_ref$total_vaccines[1],
...
prepare_data <- function(country, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, variants_to_disregard, variants_aggregated, variants_aggregated_names, daily_spline){
  ...
  df_disease_all <- df_disease_all %>%
    select(date, new_cases, total_cases, new_deaths, total_deaths, new_vaccines, total_vaccines, population)
