...
download_files_and_load_data_Italy <- function(country_long, global_initial_date, global_final_date, variants, variants_to_disregard = list(), variants_aggregated = list(), variants_aggregated_names = list()){
  ...
  if(variants){
    ...
    df_variants_init <- filter_variants_Italy(df_variants_init)
...
filter_variants_Italy <- function(df_variants_init){
...
compute_data_Italy <- function(df_disease_ref, df_variants_ref, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, daily_spline, country){
  # Preprocess data
...
#   - df_variants_ref:        dataframe with variants data (after preprocessing)
#   - df_disease_ref:         dataframe with disease data (after preprocessing)
prepare_data_Italy <- function(country, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, variants_to_disregard, variants_aggregated, variants_aggregated_names, daily_spline){
  # Download file and load data
  data <- download_files_and_load_data_Italy(country, global_initial_date, global_final_date, variants, variants_to_disregard, variants_aggregated, variants_aggregated_names)
  df_disease_init <- data[[1]]
  df_variants_init <- data[[2]]

  data <- compute_data_Italy(df_disease_init, df_variants_init, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, daily_spline, country)
  if(is.null(data))
    return(NULL)
