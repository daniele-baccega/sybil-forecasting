...
compartmental_models <- function(SIRDS_initial_marking, dir_name, df_disease_ref, df_variants_ref, immunization_end_rate, recovery_rate, recovery_data){
  ...
  if(new_data){
    ...
    if(!daily_spline){
      ...
      S_local_weekly <- I_local_weekly <- R_local_weekly <- D_local_weekly <- V_local_weekly <- rep(0, n_weekly)
      ...
      V_local_weekly[1] <- SIRDS_initial_marking[5]
      ...
      if(recovery_data){
        for(t in 2:n_weekly){
          S_local_weekly[t] <- S_local_weekly[t-1] - df_disease_ref$new_cases[t] + R_local_weekly[t-1] * immunization_end_rate_weekly - df_disease_ref$new_vaccines[t] + V_local_weekly[t-1] * immunization_end_rate
          ...
          V_local_weekly[t] <- V_local_weekly[t-1] + df_disease_ref$new_vaccines[t] - V_local_weekly[t-1] * immunization_end_rate
        }
      }
      else{
        for(t in 2:n_weekly){
      ...
      V_spline <- splinefun(seq(1, n_weekly*7, 7), V_local_weekly, method = "monoH.FC")
...
get_rates <- function(SIRD, after_date_SIRD, immunization_end_rate, N){
  ...
  results_all <- data.frame(date=SIRD$date, infection_rates, rec_rates, fat_rates, vac_rates)
...
#   - values:                   training values
#   - suffix:                   suffix
apply_NeuralProphet <- function(dir_name, date, values, time_step, suffix){
  prophet_df <- data.frame(ds=date, y=values)
  
  if(!file.exists(paste0(dir_name, "neural_prophet_models/neural_prophet_data_", suffix, ".csv"))){
    write.csv(prophet_df, paste0(dir_name, "neural_prophet_models/neural_prophet_data_", suffix, ".csv"), row.names = FALSE)
    
    system(paste0("bash NeuralProphet.sh ", dir_name, "neural_prophet_models/ ", suffix))
  }
  
  data <- read.csv(paste0(dir_name, "neural_prophet_models/forecast_", time_step, "_", suffix, ".csv"))[, 2]
  
  forecast <- data.frame(yhat=data, yhat_lower=data, yhat_upper=data)
...
SIRD_det <- function(n, n_ref, N, SIRD, infection_rates, global_infection_rates, global_recovery_rates, global_fatality_rates, global_vaccination_rates, immunization_end_rate, variants, time_step){
  if(variants){
    SIRD_all_variants <- SIRD %>%
      group_by(date) %>%
      summarize(S=first(S), I=sum(I), R=first(R), D=first(D), V=first(V))
    
    n <- nrow(SIRD_all_variants)
    n_ref <- n + time_step
    
    S_local_all_variants <- I_local_all_variants <- R_local_all_variants <- D_local_all_variants <- V_local_all_variants <- rep(NA, n_ref)
    ...
    SIRD_ev <- data.frame(date=seq.Date(SIRD_all_variants$date[1], SIRD_all_variants$date[n] + time_step, 1), S=rep(S_local_all_variants, length(variants_name)), I=I_local$I, R=rep(R_local_all_variants, length(variants_name)), D=rep(D_local_all_variants, length(variants_name)), V=rep(V_local_all_variants, length(variants_name)), variant=I_local$variant)
  }
  else{
    S_local <- I_local <- R_local <- D_local <- V_local <- rep(NA, n_ref)
    ...
    SIRD_ev <- data.frame(date=seq.Date(SIRD$date[1], SIRD$date[n] + time_step, 1), S=S_local, I=I_local, R=R_local, D=D_local, V=V_local)
  }
