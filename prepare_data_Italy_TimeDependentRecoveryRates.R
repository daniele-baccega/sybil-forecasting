...
download_files_and_load_data <- function(country_long, global_initial_date, global_final_date, variants, variants_to_disregard = list(), variants_aggregated = list(), variants_aggregated_names = list()){
  ...
  df_disease_ref_init <- df_disease_ref_init %>%
    mutate(confirmed = na.approx(confirmed, na.rm = FALSE), recovered = na.approx(recovered, na.rm = FALSE), deaths = na.approx(deaths, na.rm = FALSE))
  
  df_disease_ref_init <- df_disease_ref_init %>%
    filter(!is.na(confirmed), !is.na(recovered), !is.na(deaths), date <= global_final_date, date >= global_initial_date)
...
compute_data <- function(df_disease_ref, df_variants_ref, global_initial_date, global_final_date, immunization_end_rate, recovery_rate, variants, daily_spline, country){
  ...
  df_disease_ref <- df_disease_ref %>%
    mutate(total_deaths = deaths, total_cases = confirmed, total_recoveries = recovered) %>%
    select(date, total_deaths, total_recoveries, total_cases, population) %>%
    mutate(new_deaths = diff(c(0, total_deaths)), new_cases = diff(c(0, total_cases)), new_recoveries = diff(c(0, total_recoveries))) %>%
