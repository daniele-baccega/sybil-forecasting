...
Sybil <- function(variants = TRUE, daily_spline = FALSE, global_final_date = as.Date("2023-06-04"), country = "Italy", external_dir_names = paste0("Scenario_", as.numeric(Sys.time())), immunization_end_rate = 1 / 180, recovery_rate = 1 / 10, reproduce = FALSE, forecast = FALSE, initial_dates = c(), final_dates = c(), variants_to_disregard = list(), variants_aggregated = list(), variants_aggregated_names = list()){
  ...
  internal_dir_name <- paste0("SIRD", if(variants) "_Variants" else "", if(immunization_end_rate > 0) "_Reinfection" else "", "_MCMC", mcmc_samples, if(daily_spline) "_DailySpline" else "", "/")
  ...
  for(j in seq(1, length(external_dir_names))){
    ...
    data <- compute_data(dir_name, df_COVID19_init, df_variants_init, immunization_end_rate, recovery_rate, global_final_date, !updated_file || reproduce)
    ...
    save_rates(dir_name, results_all, results_all_variants)
    ...
    if(forecast){
      ...
      plot_I(dir_name, SIRD_all_spline, "spline")
